...
class ItemHistoryService extends MainService
{
    ...
    public function all($fields = null,$active=1)
    {
        ...
        return ItemHistory::active($active)->get($data);
    }
    ...
    {
        $search = trim($search);
        return empty($search) ? ItemHistory::query()
            : ItemHistory::query()->where('name', 'like', '%' . $search . '%')

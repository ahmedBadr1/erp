...
class NodesController extends ApiController
{
    public function __construct()
    {
        ...
        $this->middleware('permission:accounting.nodes.view', ['only' => ['index', 'show']]);
        $this->middleware('permission:accounting.nodes.create', ['only' => ['create', 'store']]);
        $this->middleware('permission:accounting.nodes.edit', ['only' => ['edit', 'update']]);
        $this->middleware('permission:accounting.nodes.delete', ['only' => ['destroy']]);
    ...
    {
        if (auth('api')->user()->cannot('accounting.nodes.index')) {
            return $this->deniedResponse(null, null, 403);
        }
        $nodes = Node::active()->get(['name', 'code', 'credit', 'system', 'active']);
        if ($request->has('type')) {
